...
public abstract class RequestFacade implements Closeable {
	...
	/**
	 * Returns the text associated with the part with the given name. If
	 * the decorated request has no part with the given name,
	 * {@link Optional#empty()} is returned.
	 *
	 * @param partName
	 *            the name of the part with the desired input stream.
	 * @return a string containing the text associated with the part with the given name. If
	 *         the decorated request has no part with the given name,
	 *         {@link Optional#empty()} is returned.
	 * @throws IOException
	 * @throws ServletException
	 */
	public Optional<String> getPartText(String partName) throws IOException, ServletException {
		Part part = request().getPart(partName);
		if (part == null) {
			return Optional.empty();
		}
		final InputStream ret = part.getInputStream();
		return Optional.of(IO.toString(ret));
	}
